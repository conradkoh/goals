...
/**
 * V2 Industrial Design System - Card Component
 *
 * Features:
 * - Sharp corners (no border-radius)
 * - Glassmorphism with backdrop-blur
 * - 2px borders with low opacity
...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn(
        'bg-card/60 backdrop-blur-sm text-card-foreground flex flex-col gap-4 border-2 border-border/10 p-4 shadow-sm',
        className
      )}
      {...props}
    />
