...
import { AdhocGoalsList } from '@/components/molecules/AdhocGoalsList';
...
export function AdhocGoalsSection({
  ...
  const completedWeekGoals = completedAdhocGoals.filter(
    (goal) => goal.adhoc?.weekNumber === weekNumber
  );

  // Combine real and optimistic goals (only for incomplete)
  const allIncompleteGoals = [...optimisticGoals, ...incompleteWeekGoals];
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createAdhocGoal(
        ...
        weekNumber,
        undefined, // No dayOfWeek - adhoc tasks are week-level only
        undefined
      );
  ...
  const content = (
    <Tabs defaultValue="active" className="w-full">
      <TabsList className="grid w-full grid-cols-2">
        <TabsTrigger value="active">Active ({allIncompleteGoals.length})</TabsTrigger>
        <TabsTrigger value="completed">Completed ({completedWeekGoals.length})</TabsTrigger>
      </TabsList>

      <TabsContent value="active" className="mt-4 space-y-2">
        <AdhocGoalsList
          goals={allIncompleteGoals}
