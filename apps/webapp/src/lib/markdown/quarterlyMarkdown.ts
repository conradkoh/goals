import { QuarterlyGoalSummary, MultipleQuarterlyGoalsSummary } from '@services/backend/src/usecase/getWeekDetails';
...
export function generateMultipleQuarterlyGoalsMarkdown(
  ...
  lines.push(`- **Quarterly Goals:** ${completedQuarterlyGoals}/${quarterlyGoals.length} completed`);
  ...
  quarterlyGoals.forEach((summary, index) => {
    ...
    if (omitIncomplete && !quarterlyGoal.isComplete) {
      const hasCompletedSubGoals = Object.values(weeklyGoalsByWeek).flat().some(weekly => 
        weekly.isComplete || (weekly.children && weekly.children.some(daily => daily.isComplete))
      );
    ...
    const goalTotalDailyGoals = allWeeklyGoals.reduce((sum, weeklyGoal) => sum + (weeklyGoal.children?.length || 0), 0);
